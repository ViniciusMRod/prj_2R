...
class PCMSOExtractor:
    ...
    def extract_cargos(self):
        ...
        if match:
            ...
            for line in lines:
                ...
                # Ignorar cabeçalhos de tabela que podem ser confundidos com setores
                ...
                if (('EST' in line and '-' in line) or 
                    ...
                    # Remove quantidade (números no final da linha) e limpa o nome do cargo
                    cargo_nome = re.sub(r'[\s\-_]+(?:\(?\d+\)?)$', '', line).strip().upper()
                    ...
                    if (len(cargo_nome) > 3 and 
                        ...
                        'QUADRO' not in cargo_nome and
                        not cargo_nome.isdigit()):
    ...
    def extract_riscos(self):
        """Extrai riscos com classificação automática (sem coluna de danos à saúde).

        Estratégia:
        1) Tenta primeiro extrair via TABELA (pdfplumber) para preservar estrutura e ordem,
           mesmo quando houver células mescladas e quebras de linha.
        2) Fallback para regex no texto corrido.
        Output (mantido): lista de dicts {'tipo': <str>, 'risco': <str>}
        """
        ...
        def classify_tipo(risco_desc: str) -> str:
            tipo = 'GERAL'
            risco_up = (risco_desc or "").upper()
            for tipo_chave, palavras in self.TIPO_RISCO_MAP.items():
                if any(palavra in risco_up for palavra in palavras):
                    tipo = tipo_chave
                    break
            return tipo

        def parse_riscos_from_table(table_rows):
            riscos = []
            for row in table_rows:
                ...
                if "RISCO" in row_text and ("DANO" in row_text or "SAÚDE" in row_text or "SAUDE" in row_text):
                    continue
                if row_text.strip() in {"RISCO", "RISCOS"}:
                    continue

                # Heurística de coluna: normalmente o risco está na 1ª coluna "Risco"
                # (às vezes a detecção de colunas falha e colapsa tudo; nesse caso usamos a linha toda)
                risco_desc = cells[0] if cells else ""
                if len(cells) == 1:
                    risco_desc = cells[0]
                elif len(cells) >= 2 and (len(cells[0]) < 3 or cells[0].isdigit()):
                    # quando a primeira coluna vira índice/ordem, risco tende a estar na segunda
                    risco_desc = cells[1]

                risco_desc = self._clean_text(risco_desc)

                if risco_desc and len(risco_desc) > 5:
                    riscos.append({
                        'tipo': classify_tipo(risco_desc),
        ...
        # 1) tentativa por tabela em páginas que mencionam riscos
        pages_with_riscos = []
        for i, page in enumerate(self.pdf.pages):
            txt = self._filter_provider_header(page.extract_text() or "")
            if re.search(r"\bRISCOS?\b", txt, re.IGNORECASE):
                pages_with_riscos.append(i)
        ...
        for pno in pages_with_riscos:
            ...
            for t in tables:
                if not t or len(t) < 2:
                    continue
                extracted.extend(parse_riscos_from_table(t))

        # 2) fallback por regex no texto
        if not extracted:
            text = self.full_text
            # captura linhas iniciadas por "Risco:"; ignora blocos de "Danos à Saúde"
            pattern = r'Risco[:\s]+([^\n]+?)(?:\n|$)'
        ...
        seen = set()
        unique = []
        for risco in extracted:
            key = (risco.get('tipo'), risco.get('risco'))
            if key not in seen:
                seen.add(key)
                unique.append(risco)
